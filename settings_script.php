...
if ($new_pass != $new_pass1) {
	$error = "<span class='red'>Passwords don't match</span>";
    header('location: settings.php?error= ' . $error);
} else {
    if ($old_pass == $orig_pass) {
        $query = "UPDATE  users SET password = '" . $new_pass . "' WHERE email = '" . $_SESSION['email'] . "'";
        mysqli_query($con, $query) or die($mysqli_error($con));
        $error = "<span class='red'>Yoyr New Password Updated</span>";
        header('location: settings.php?error= ' . $error);
    } else{
    	$error = "<span class='red'>Your Old Password is Wrong</span>";
        header('location: settings.php?error= ' . $error);
    }
}
?>